...
void embed_parser(const char *str)
{
    while (true)
    {
        ...
        case 'I':
            ...
            break;
        case 'j':
            param.embed_javascript = 0;
            break;
        case 'J':
            param.embed_javascript = 1;
            break;
        case 'o':
            param.embed_outline = 0;
            break;
        case 'O':
...
int main(int argc, char **argv)
{
    ...
    param.poppler_data_dir = POPPLER_DATA_PATH;
    ...
    {
        {
            ...
            // TODO: convert to use std types instead of doing this wrapping/unwrapping
            auto ownerPWOpt = std::optional<GooString>{ownerPW};
            auto userPWOpt = std::optional<GooString>{userPW};
            std::unique_ptr<PDFDoc> pdfDoc = PDFDocFactory().createPDFDoc(fileName, ownerPWOpt, userPWOpt);
            doc = pdfDoc.release();

            // cleanup password strings
            delete userPW;
            delete ownerPW;
            ownerPWOpt.reset();
            userPWOpt.reset();
