...
string HTMLRenderer::dump_type3_font (GfxFont * font, FontInfo & info)
{
    ...
    // TODO: refactor this method to not take in raw pointer
    // so we don't have to do this wrapping here
    std::shared_ptr<GfxFont> sharedPtrFont (font);
    ...
    std::shared_ptr<CairoFont> cur_font = font_engine.getFont(sharedPtrFont, cur_doc, true, xref);
    ...
    if(font_8bit)
    {
        ...
        if(is_truetype_suffix(suffix))
        {
            ...
            else
            {
                ...
                {
                    FoFiTrueType* fftt_ptr = fftt.release();
                    code2GID = font_8bit->getCodeToGIDMap(fftt_ptr);
                    code2GID_len = 256;
                    delete fftt_ptr;
                }
            }
    ...
    else
    {
        ...
        if(is_truetype_suffix(suffix))
        {
            ...
            else
            {
                ...
                {
                    FoFiTrueType* fftt_ptr = fftt.release();
                    code2GID = _font->getCodeToGIDMap(fftt_ptr, &code2GID_len);
                    delete fftt_ptr;
                }
            }
        }
...
const FontInfo * HTMLRenderer::install_font(GfxFont * font)
{
     ...
     */
    if(std::optional<GfxFontLoc> font_loc = font->locateFont(xref, nullptr))
    {
